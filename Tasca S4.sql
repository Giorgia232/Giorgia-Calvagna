...
WHERE FIND_IN_SET(p.id, t.product_ids) > 0;

SELECT * FROM sold_products;

ALTER TABLE transactions DROP COLUMN product_ids;

SELECT p.id AS product_id, p.product_name, COUNT(sp.transaction_id) AS num_sales
FROM products p
LEFT JOIN sold_products sp 
ON p.id = sp.product_id
LEFT JOIN transactions t 
ON sp.transaction_id = t.id AND t.declined = 0
